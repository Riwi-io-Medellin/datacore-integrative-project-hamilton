...
```
Campuss (2) → Cohorts (7) → Clans (35) → Couders (N)
...
**Datos Iniciales**:
```sql
INSERT INTO campuses(name) VALUES ('Medellín'), ('Barranquilla');
```
...
#### intervention_types
**Propósito**: Clasificación de interventions clínicas
```sql
...
#### cohorts
**Propósito**: Cohorts formativos por sede y ruta
```sql
CREATE TABLE cohorts (
    id          SERIAL  PRIMARY KEY,
    name        VARCHAR(100) NOT NULL,
    sede_id     INT NOT NULL REFERENCES campuses(id) ON DELETE RESTRICT,
...
| sede_id | INT | NOT NULL, FK | campuses.id | Campus asignada |
| route_id | INT | NOT NULL, FK | routes.id | Track formativa |
| is_active | BOOLEAN | NOT NULL | - | Status del cohort |
| created_at | TIMESTAMPTZ | NOT NULL | - | Fecha de creación |

**Relaciones**:
- `sede_id` → `campuses(id)` (ON DELETE RESTRICT)
...
| shift | VARCHAR(10) | NOT NULL, CHECK | - | Shift (morning/afternoon) |
...
| is_active | BOOLEAN | NOT NULL | - | Status del usuario |
| created_at | TIMESTAMPTZ | NOT NULL | - | Fecha de creación |
...
| status | VARCHAR(20) | NOT NULL, CHECK | - | Status del couder |
...
### 3.4 Tablas de Interventions y Análisis

#### interventions
**Propósito**: Registro de interventions clínicas
```sql
...
| summary | TEXT | NOT NULL | - | Summary del análisis |
| diagnosis | TEXT | NOT NULL | - | Diagnosis |
| suggestions | TEXT | NOT NULL | - | Suggestions |
| created_at | TIMESTAMPTZ | NOT NULL | - | Fecha de creación |
...
```
campuses (1) ←→ (N) cohorts (1) ←→ (N) clans (1) ←→ (N) couders
  │              │              │              │
  │              │              │              └── (1) ←→ (N) interventions
  │              │              │              └── (1) ←→ (N) ai_analyses
  │              │              │
...
|-------|----------------|-----------------|-------|
| cohorts.sede_id | campuses.id | RESTRICT | Proteger cohorts al eliminar sede |
...
| interventions.couder_id | couders.id | CASCADE | Eliminar interventions si se elimina couder |
| interventions.user_id | users.id | RESTRICT | Proteger interventions al eliminar usuario |
| interventions.intervention_type_id | intervention_types.id | RESTRICT | Proteger interventions al eliminar tipo |
...
CREATE INDEX pk_campuses ON campuses(id);
...
CREATE INDEX idx_couders_national_id   ON couders(national_id);     -- Search por CC
CREATE INDEX idx_couders_clan_id       ON couders(clan_id);         -- Listar por clan
CREATE INDEX idx_couders_status        ON couders(status);          -- Filter por estado
CREATE INDEX idx_interventions_couder  ON interventions(couder_id); -- Historial del couder
CREATE INDEX idx_interventions_user    ON interventions(user_id);    -- Interventions por usuario
...
-- Para historial de interventions ordenado
...
-- Nombre único en campuses
ALTER TABLE campuses ADD CONSTRAINT uniq_sede_name UNIQUE (name);
...
JOIN campuses s ON c.sede_id = s.id
...
HAVING COUNT(i.id) >= 3  -- Al menos 3 interventions
...
1. **Search de couders por CC** (idx_couders_national_id)
2. **Historial de interventions** (idx_interventions_couder_date)
