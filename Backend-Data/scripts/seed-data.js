...
async function seedDatabase() {
    ...
    try {
        ...
        for (const s of sedeDefs) {
            const result = await client.query(`
                INSERT INTO campuses (name) VALUES ($1)
                ON CONFLICT (name) DO UPDATE SET name = EXCLUDED.name
                RETURNING id
            `, [s.name]);
            if (result.rows.length > 0) campuses.push({ id: result.rows[0].id, name: s.name });
        }
        console.log(`[seed] [OK] ${campuses.length} campuses ready`);
        ...
        if (campuses.length >= 2) {
            cohortDefs.push(
                { name: 'Cohort 2024-1',        route: 'basica',   sede: campuses[0].id },
                { name: 'Cohort 2024-2',        route: 'basica',   sede: campuses[0].id },
                { name: 'Cohort 2024-Avanzado', route: 'avanzada', sede: campuses[0].id },
                { name: 'Cohort 2025-1',        route: 'basica',   sede: campuses[0].id },
                { name: 'Cohort 2024-1',        route: 'basica',   sede: campuses[1].id },
                { name: 'Cohort 2024-Avanzado', route: 'avanzada', sede: campuses[1].id }
            );
        }
            ...
            for (let i = 0; i < numIv; i++) {
                ...
                switch (type) {
                    case 'initial_evaluation':
                        notes = `Evaluacion inicial de ${couder.full_name}. Muestra buena disposicion. Score: ${couder.average_score}.`;
                        break;
                    case 'follow_up':
                        notes = `Follow-up de ${couder.full_name}. Avance en actividades. Status: ${couder.status}.`;
                        break;
                    case 'risk_assessment':
                        notes = `Evaluacion de riesgo para ${couder.full_name}. Se identifican areas de oportunidad.`;
                        break;
                    case 'closing':
                        notes = `Closure de intervencion para ${couder.full_name}. Score final: ${couder.average_score}.`;
                        break;
                    default:
        ...
        for (const couder of activeCouders) {
            const summary = `Analisis de ${couder.full_name}. Score promedio: ${couder.average_score}. Buena adaptacion al programa.`;
