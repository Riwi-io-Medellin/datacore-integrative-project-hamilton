...
async function seed() {
    ...
    try {
        ...
        const { rows: sedeRows } = await client.query('SELECT id, name FROM campuses');
        const campuses = Object.fromEntries(sedeRows.map(s => [s.name, s.id]));
        ...
        // ── Cohorts (Cohorts) ────────────────────────────────────────
        const cohortData = [
            { name: 'Cohort 4', sede: 'Medellín',    route: 'basica' },
            { name: 'Cohort 6', sede: 'Medellín',    route: 'avanzada' },
            { name: 'Cohort 5', sede: 'Barranquilla', route: 'basica' },
            { name: 'Cohort 7', sede: 'Barranquilla', route: 'avanzada' },
        ];
        ...
        // ── Clans ───────────────────────────────────────────────────
        const clanData = [
            // Medellín Cohort 4 (Basic)
            { name: 'Clan A', cohort: 'Cohort 4', shift: 'morning',   tl: 'Valentina Ríos' },
            { name: 'Clan B', cohort: 'Cohort 4', shift: 'afternoon', tl: 'Carlos Mora' },
            { name: 'Clan C', cohort: 'Cohort 4', shift: 'morning',   tl: 'Sofía Castro' },
            // Medellín Cohort 6 (Advanced)
            { name: 'Clan A', cohort: 'Cohort 6', shift: 'morning',   tl: 'Daniel Pérez' },
            { name: 'Clan B', cohort: 'Cohort 6', shift: 'afternoon', tl: 'Laura Gómez' },
            { name: 'Clan C', cohort: 'Cohort 6', shift: 'morning',   tl: 'Miguel Ángel' },
            // Barranquilla Cohort 5 (Basic)
            { name: 'Clan A', cohort: 'Cohort 5', shift: 'morning',   tl: 'Miguel Torres' },
            { name: 'Clan B', cohort: 'Cohort 5', shift: 'afternoon', tl: 'Isabela Vargas' },
            { name: 'Clan C', cohort: 'Cohort 5', shift: 'morning',   tl: 'Andrés López' },
            { name: 'Clan D', cohort: 'Cohort 5', shift: 'afternoon', tl: 'Camila Ruiz' },
            // Barranquilla Cohort 7 (Advanced)
            { name: 'Clan A', cohort: 'Cohort 7', shift: 'morning',   tl: 'Felipe Díaz' },
            { name: 'Clan B', cohort: 'Cohort 7', shift: 'afternoon', tl: 'Lucía Martínez' },
            { name: 'Clan C', cohort: 'Cohort 7', shift: 'morning',   tl: 'Roberto Silva' },
        ];
        ...
        const interventionNotes = [
            ...
            'Follow-up de avances. Se evidencia mejora en la participación. Se trabajaron estrategias de gestión del tiempo.',
        ...
        for (let i = 0; i < aiCouders.length; i++) {
            ...
            `, [
                couder.id,
                `Análisis ${i + 1}`,
                `Análisis comprehensivo del couder basado en interventions registradas durante el periodo formativo.`,
                aiDiagnoses[i],
                aiSuggestions[i]
            ]);
        }
