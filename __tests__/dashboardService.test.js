...
const CampusRepository    = require('../src/repositories/sedeRepository');
...
describe('DashboardService', () => {
    ...
    test('getGlobalDashboard returns global stats and campuses', async () => {
        CampusRepository.findAll.mockResolvedValue([{ id: 1, name: 'Medellín' }]);
        CampusRepository.globalStats.mockResolvedValue({ total: '100', active: '80', withdrawn: '10', completed: '10' });
        CouderRepository.statsByCampus.mockResolvedValue({ total: '50', active: '40', withdrawn: '5', completed: '5' });
        CouderRepository.attendedCountByCampus.mockResolvedValue(30);
        CohortRepository.findByCampusId.mockResolvedValue([{ id: 1, name: 'Cohort 1' }]);
        ...
        expect(result.campuses[0].attendancePercent).toBe(60);
    });

    test('getCohortsByCampus throws NotFoundError for unknown sede', async () => {
        CampusRepository.findById.mockResolvedValue(null);
        await expect(DashboardService.getCohortsByCampus(999)).rejects.toBeInstanceOf(NotFoundError);
    });
