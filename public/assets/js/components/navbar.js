...
const Navbar = (() => {
                    ...
                    <div class="flex items-center gap-4">
                        ...
                        <div class="flex items-center gap-2 bg-gray-700 rounded-lg px-3 py-2 border border-gray-600">
                            ...
                            <input id="global-search-input"
                                   type="text"
                                   placeholder="Search Couder by ID..."
    ...
    function _assignEventListeners() {
        ...
        // Campuss button
        const campusesBtn = document.createElement('button');
        campusesBtn.className = 'flex items-center gap-1.5 text-sm text-gray-300 hover:text-white transition-colors px-3 py-2 rounded-lg hover:bg-gray-700';
        campusesBtn.innerHTML = `
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4"/>
            </svg>
        `;
        
        // Insert campuses button after search button
        if (dashboardBtn && searchIcon) {
            const parent = searchIcon.parentNode;
            parent.insertBefore(campusesBtn, searchIcon.nextSibling);
        }
        
        // Campuss button click event
        if (campusesBtn) {
            campusesBtn.addEventListener('click', () => {
                console.log('Campuss button clicked, navigating to campuses');
                Router.navigate('sede');
            });
            console.log('Campuss button listener assigned');
        }
