/* =============================================================
   sede.js — Campus view with cohorts - FULL INTERACTIVITY
...
const CampusView = (() => {
    ...
    async function render(params) {
        try {
                    ...
                    <nav class="bg-gray-800/90 backdrop-blur-sm border-b border-gray-700/50 px-6 py-4 sticky top-0 z-50">
                        <div class="flex items-center justify-between">
                            ...
                            <div class="flex items-center gap-3">
                                <button onclick="CampusView._refreshData()" class="bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg transition-all hover:scale-105 transform flex items-center gap-2">
                    ...
                    <div class="p-6">
                        <div id="sede-content">
                            <div class="text-center py-12">
                                ...
                                <h2 class="text-xl font-semibold text-white mb-2">Cargando Campus...</h2>
                                <p class="text-gray-400">Fetching campus information and cohorts</p>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            const data = await Api.get(`/dashboard/campuses/${sedeId}`);
            console.log('API Response:', data);
            _sedeData = data.sede;
            _cohortsData = data.cohorts || [];
            renderCampusWithCohorts(data);
                ...
                app.innerHTML = `
                    <div class="min-h-screen bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 text-white flex items-center justify-center">
                        <div class="text-center">
                            ...
                            <h2 class="text-2xl font-bold text-red-400 mb-4">Error al cargar Campus</h2>
    ...
    function renderCampusWithCohorts({ sede, cohorts }) {
                    ...
                    <div class="flex items-center justify-between mb-8">
                        <div>
                            <div class="flex items-center gap-3 mb-4">
                                ...
                                <h1 class="text-4xl font-bold bg-gradient-to-r from-blue-400 to-purple-400 bg-clip-text text-transparent">Cohorts ${sede.name || 'Campus'}</h1>
                            </div>
                            <p class="text-gray-400 text-lg">Cohorts available at ${sede.name || 'esta sede'}</p>
                        </div>
                        <div class="bg-blue-600/80 backdrop-blur-sm px-4 py-2 rounded-lg group-hover:scale-110 transition-transform">
                            <span class="text-white font-medium">Campus Active</span>
                        </div>
                    </div>
                        ...
                        <div class="bg-gray-700/50 backdrop-blur-sm rounded-lg p-6 border border-gray-600/50 group/item hover:bg-gray-700/70 transition-all">
                            ...
                            <div class="space-y-3">
                                <div class="flex items-center justify-between group/subitem">
                                    ...
                                    <span class="text-white font-medium group/subitem:text-blue-300 transition-colors">${sede.address || 'Not available'}</span>
                                </div>
                                <div class="flex items-center justify-between group/subitem">
                                    <span class="text-gray-400 group/subitem:text-gray-300 transition-colors">Phone:</span>
                                    <span class="text-white font-medium group/subitem:text-blue-300 transition-colors">${sede.phone || 'Not available'}</span>
                                </div>
                                <div class="flex items-center justify-between group/subitem">
                                    <span class="text-gray-400 group/subitem:text-gray-300 transition-colors">Status:</span>
            ...
            <div class="mb-6">
                <div class="flex items-center justify-between mb-4">
                    <div class="flex items-center gap-3">
                        ...
                        <span class="bg-blue-600/20 text-blue-400 px-3 py-1 rounded-full text-sm">${cohorts.length} cohorts</span>
                    </div>
                    <div class="flex gap-2">
                        <button onclick="CampusView._sortCohorts('name')" class="text-gray-400 hover:text-white transition-colors px-3 py-2 rounded-lg hover:bg-gray-700/50">
                            <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 4h13M3 8h9m-9 4h6m4 0l4-4m0 0l4 4m-4-4v12"/>
                            </svg>
                            Sort
                        </button>
                        <button onclick="CampusView._filterCohorts('all')" class="text-gray-400 hover:text-white transition-colors px-3 py-2 rounded-lg hover:bg-gray-700/50">
                            <svg class="w-4 h-4 inline mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <div class="cohort-card group bg-gray-800/50 backdrop-blur-sm rounded-xl p-6 border border-gray-700/50 hover:border-purple-600/50 transition-all duration-300 hover:scale-105 hover:shadow-2xl cursor-pointer relative" 
                        ...
                        <div class="grid grid-cols-3 gap-4">
                            ...
                            <div class="text-center">
                                ...
                                <div class="text-xs text-gray-400">Attended</div>
                            </div>
                        </div>
                    </div>
                    `).join('')}
    ...
    async function _refreshData() {
        ...
        btn.innerHTML = '<div class="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div> Refreshing...';
        btn.disabled = true;
        
        try {
            const data = await Api.get(`/dashboard/campuses/${_sedeData.id}`);
            _sedeData = data.sede;
            _cohortsData = data.cohorts || [];
            renderCampusWithCohorts(data);
    ...
    function _sortCohorts(sortBy) {
        ...
        renderCampusWithCohorts({ sede: _sedeData, cohorts: _cohortsData });
