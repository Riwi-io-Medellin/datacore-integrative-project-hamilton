...
const CouderView = (() => {
        ...
        content.innerHTML =
            ...
            '<div class="bg-gray-800/60 rounded-xl p-8 border border-gray-700/50 mb-8">' +
              ...
              '<div class="grid grid-cols-2 md:grid-cols-4 gap-4">' +
                _cell('Campus', _couder.sede_name) +
                _cell('Cohort', _couder.cohort_name) +
                _cell('Clan', _couder.clan_name) +
                _cell('Track', _couder.route_name === 'avanzada' ? 'Advanced' : 'Basic') +
            ...
            '<div class="bg-gray-800/60 rounded-xl p-8 border border-gray-700/50 mb-8">' +
              ...
              '<div class="space-y-4">' +
                '<div><label class="block text-gray-300 mb-2 text-sm font-medium">Intervention Type *</label>' +
            ...
            '<div class="bg-gray-800/60 rounded-xl p-8 border border-gray-700/50 mb-8">' +
              '<div class="flex items-center justify-between mb-6">' +
                ...
                '<button id="btn-generate-ai" class="bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 px-5 py-2.5 rounded-lg transition-all hover:scale-105 text-white font-medium flex items-center gap-2">' +
                  '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 10V3L4 14h7v7l9-11h-7z"/></svg>Generate Analysis' +
                '</button>' +
              '</div>' +
              (_aiAnalyses.length === 0
                ? '<p class="text-gray-500 text-center py-6">No analyses generated yet. Generate one using the button above.</p>'
            ...
            '<div class="bg-gray-800/60 rounded-xl p-8 border border-gray-700/50 mb-8">' +
              '<h3 class="text-xl font-bold text-white mb-6 flex items-center gap-3">' +
                ...
                'Historial de Interventions <span class="bg-blue-600/20 text-blue-400 px-3 py-1 rounded-full text-sm font-normal ml-2">' + _interventions.length + '</span>' +
              '</h3>' +
              (_interventions.length === 0
                ? '<p class="text-gray-500 text-center py-6">Sin interventions registradas aún.</p>'
    ...
    function _setupEventListeners() {
        ...
        _aiAnalyses.forEach((a, idx) => {
            ...
            if (ttsBtn) {
                const ttsText = 'Summary: ' + (a.summary || '') + '. Diagnosis: ' + (a.diagnosis || '') + '. Suggestions: ' + (Array.isArray(a.suggestions) ? a.suggestions.join('\n') : (a.suggestions || ''));
    ...
    async function _submitIntervention() {
        ...
        if (!typeId) return showErr('Please select the intervention type.');
    ...
    async function _generateAIAnalysis() {
        if (_interventions.length === 0) {
            Toast.show('Este couder no tiene interventions registradas aún.', 'error');
            return;
        }
    ...
    function _toggleTTS(idx, text) {
        ...
        if (!window.speechSynthesis) {
            Toast.show('Your browser does not support speech synthesis.', 'error');
            return;
        }
        ...
        utt.onstart  = () => { _ttsPlaying = idx; if (btn) btn.querySelector('span').textContent = 'Stop'; };
        utt.onend = utt.onerror = () => { _ttsPlaying = null; if (btn) btn.querySelector('span').textContent = 'Listen'; };
            ...
            '<nav class="bg-gray-800/90 backdrop-blur-sm border-b border-gray-700/50 px-6 py-4 sticky top-0 z-50">' +
              '<div class="flex items-center justify-between">' +
                ...
                '<button onclick="Auth.logout()" class="bg-red-600 hover:bg-red-700 px-4 py-2 rounded-lg flex items-center gap-2">' +
                  '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>Sign out</button>' +
              '</div>' +
            '</nav>' +
            '<div class="p-6">' +
              '<div class="flex items-center gap-4 mb-6">' +
                ...
                '<button onclick="CouderView._refreshData()" class="ml-auto bg-blue-600 hover:bg-blue-700 px-4 py-2 rounded-lg flex items-center gap-2">' +
                  '<svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15"/></svg>Refresh</button>' +
              '</div>' +
