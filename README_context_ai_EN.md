...
**Sistema Clínico de Gestión de Interventions con IA**
**Fecha**: 2026-03-13  
**Versión**: 1.0  
**Status**: 100% FUNCIONAL Y VERIFICADO
...
- Registro y seguimiento de interventions clínicas
...
```
Campus (2: Medellín, Barranquilla)
└── Cohort/Cohort (7 total: 4 Medellín, 3 Barranquilla)
...
- All los usuarios tienen igual nivel de acceso
...
CREATE TABLE cohorts (
    ...
    sede_id     INT NOT NULL REFERENCES campuses(id),
...
| Método | Endpoint | Descripción | Autenticación |
|--------|----------|-------------|---------------|
| GET | `/api/couders/search?cc={cedula}` | Search por CC | Sí |
...
| PUT | `/api/couders/{id}` | Refresh couder | Sí |
...
| PUT | `/api/interventions/{id}` | Refresh intervención | Sí |
...
| GET | `/api/dashboard/campuses/{sedeId}` | Cohorts por sede | Sí |
...
Analiza el siguiente historial de interventions y proporciona:
...
```javascript
const response = {
    summary: "Summary del progreso y patrones identificados",
...
- **Solo historial clínico**: Fechas, tipos, notas de interventions
