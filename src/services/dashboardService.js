...
const CampusRepository = require('../repositories/sedeRepository');
...
const DashboardService = {
    ...
    async getGlobalDashboard() {
        const [campuses, globalStats] = await Promise.all([
            CampusRepository.findAll(),
            CampusRepository.globalStats(),
        ]);

        const sedeStats = await Promise.all(
            campuses.map(async (sede) => {
                const stats = await CouderRepository.statsByCampus(sede.id);
                const attended = await CouderRepository.attendedCountByCampus(sede.id);
                const total = parseInt(stats.total, 10) || 0;
                
                // Obtener cohorts de esta sede
                const cohorts = await CohortRepository.findByCampusId(sede.id);
    ...
    async getCohortsByCampus(sedeId) {
        const sede = await CampusRepository.findById(sedeId);
        if (!sede) throw new NotFoundError('Campus not found');

        const cohorts = await CohortRepository.findByCampusId(sedeId);
